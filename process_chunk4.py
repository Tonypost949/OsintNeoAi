...
ZIP_PATH = r"G:\OsintNeoAi\takeout_chunks\takeout-20250115T152414Z-001.zip"
EXTRACT_DIR = r"G:\OsintNeoAi\takeout_metadata_extracted"

os.makedirs(EXTRACT_DIR, exist_ok=True)

if os.path.exists(ZIP_PATH):
    print("[CHUNK 4] Extracting files...")
    with zipfile.ZipFile(ZIP_PATH, 'r') as zf:
        entries = zf.namelist()
        print(f"[CHUNK 4] Total entries: {len(entries)}")
        for item in entries:
...
tracker_data = {
    ...
    "current_batch": 4,
    "status": "CHUNK_4_COMPLETED"
}
with open(tracker_path, 'w') as tf:
    json.dump(tracker_data, tf, indent=2)

print("[CHUNK 4] Completed and updated chunk_progress_tracker.json!")
