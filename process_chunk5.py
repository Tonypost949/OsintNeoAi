...
ZIP_PATH = r"G:\OsintNeoAi\takeout_chunks\takeout-20250311T090934Z-001.zip"
EXTRACT_DIR = r"G:\OsintNeoAi\takeout_metadata_extracted"

os.makedirs(EXTRACT_DIR, exist_ok=True)

if os.path.exists(ZIP_PATH):
    print("[CHUNK 5] Extracting files...")
    with zipfile.ZipFile(ZIP_PATH, 'r') as zf:
        entries = zf.namelist()
        print(f"[CHUNK 5] Total entries: {len(entries)}")
        for item in entries:
...
tracker_data = {
    ...
    "current_batch": 5,
    "status": "CHUNK_5_COMPLETED"
}
with open(tracker_path, 'w') as tf:
    json.dump(tracker_data, tf, indent=2)

print("[CHUNK 5] Completed and updated chunk_progress_tracker.json!")
