...
ZIP_PATH = r"G:\OsintNeoAi\takeout_chunks\takeout-20250402T152414Z-001.zip"
EXTRACT_DIR = r"G:\OsintNeoAi\takeout_metadata_extracted"

os.makedirs(EXTRACT_DIR, exist_ok=True)

if os.path.exists(ZIP_PATH):
    print("[CHUNK 7] Extracting files...")
    with zipfile.ZipFile(ZIP_PATH, 'r') as zf:
        entries = zf.namelist()
        print(f"[CHUNK 7] Total entries: {len(entries)}")
        for item in entries:
...
tracker_data = {
    ...
    "completed_chunks": [
        ...
        "takeout-20250402T152414Z-001.zip"
    ],
    "current_batch": 7,
    "status": "CHUNK_7_COMPLETED"
}
with open(tracker_path, 'w') as tf:
    json.dump(tracker_data, tf, indent=2)

print("[CHUNK 7] Completed and updated chunk_progress_tracker.json!")
