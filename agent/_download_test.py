import subprocess, os, io, time, json, sys
...
WHERE mime_type NOT LIKE 'application/vnd.google-apps.folder'
  AND mime_type NOT LIKE 'video/%%'
  AND mime_type NOT LIKE 'image/%%'
  AND mime_type NOT LIKE 'audio/%%'
  AND mime_type NOT LIKE 'application/octet-stream'
LIMIT 5
"""
rows = list(bq.query(sql).result())
print(f"Testing with {len(rows)} files")

for i, r in enumerate(rows):
    print(f"[{i+1}] {r.file_name} ({r.mime_type})")
    data, fname, err = download_file(r.file_id, r.mime_type, r.file_name)
    if err:
        print(f"  FAIL: {err}")
        continue
    gcs_key = f"drive_files/{r.file_id}/{fname}"
    bucket.blob(gcs_key).upload_from_string(data)
    print(f"  -> gs://{BUCKET}/{gcs_key} ({len(data)} bytes)")
