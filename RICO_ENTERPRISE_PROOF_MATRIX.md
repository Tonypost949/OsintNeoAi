# RICO ENTERPRISE PROOF MATRIX
**Coordination & Enterprise Tracker**

*Instruction: Use one row per **specific coordination proposition**, not one row per person or document. Circumstantial evidence may support an enterprise inference, including shared objectives, financial ties, coordinated activity, community of interest, and interlocking schemes.*

| ID | Proposition to Prove | Evidence Collected | Coordination Indicator | Witness / Expert | Alternative Explanation | Status / Next Step |
|---|---|---|---|---|---|---|
| E-01 | A common objective existed | Emails, pleadings, contracts, repeated target selection | Shared goal or outcome | Sender/recipient; records custodian | Independent but similar motives | Fact / obtain originals |
| E-02 | Participants communicated about relevant acts | Messages, call logs, meeting records | Timing and subject matter align with events | Custodian; digital-forensics expert if disputed | Routine business contact | Fact / preserve metadata |
| E-03 | Participants performed complementary roles | Access logs, filings, payments, witness statements | Division of labor or task dependency | Participant or custodian | Unrelated services | Inference / identify role evidence |
| E-04 | Resources were shared | Bank records, addresses, domains, devices, templates | Common infrastructure or support | Accountant; system custodian | Common vendor or public resource | Inference / subpoena or preserve |
| E-05 | Conduct was repeated | Timeline of similar incidents | Recurrence, related methods, same targets | Investigator; pattern analyst | Coincidence or copied behavior | Pattern / expand timeline |
| E-06 | Participants knew of one another’s roles | Cross-references, introductions, referrals, acknowledgments | Awareness of broader operation | Sender/recipient; witness | Mere professional familiarity | Inference / locate corroboration |
| E-07 | Participants benefited from coordination | Transfers, reimbursements, property changes, avoided costs | Benefit follows coordinated acts | Forensic accountant | Ordinary compensation | Inference / trace funds |
| E-08 | The structure continued over time | Repeated activity, replacement actors, recurring infrastructure | Continuity despite personnel changes | Custodians; investigator | Separate episodes | Pattern / identify continuity |
| E-09 | The alleged act caused the claimed injury | Loss records, medical records, property damage, expert analysis | Direct link between act and harm | Treating/technical expert | Independent cause | Causation / obtain baseline |
| E-10 | The theory is legally sufficient | Element-by-element legal memo | Evidence maps to each required element | Counsel | Nonactionable misconduct | Legal review / narrow claims |

---

## Scoring
Use a simple 0–3 score for each row:
- **0 — Unsubstantiated:** allegation or lead only.
- **1 — Corroborated lead:** one credible source, but material alternatives remain.
- **2 — Supported inference:** multiple independent sources converge.
- **3 — Strong proof candidate:** authenticated evidence plus competent witness or expert foundation, with no substantial unexplained contradiction.

*Do not average the scores into a single “enterprise score.” A high aggregate score should not conceal a zero on attribution, intent, or causation.*

---

## Minimum Corroboration Rule
For each major coordination proposition, seek:
1. One **primary source**—original record, custodian-produced file, transaction record, or contemporaneous communication.
2. One **independent corroborator**—different custodian, witness, system, or record series.
3. One **tested alternative explanation** documented in the case file.
4. One **foundation path** identifying who can authenticate or explain the evidence.

*For fraud-based allegations, preserve the details needed to identify the relevant actor, statement or act, time, place, method, and why it was false or wrongful; Rule 9(b) requires the circumstances of fraud to be pleaded with particularity, even though mental states may generally be alleged.*

---

## Case-File Status Labels
Use controlled labels rather than argumentative descriptions:
- **FACT:** Directly supported by a reliable source.
- **CORROBORATED FACT:** Supported by independent sources.
- **INFERENCE:** Reasonable interpretation, not directly established.
- **DISPUTED:** Conflicting evidence exists.
- **LEAD:** Requires investigation.
- **EXPERT-DEPENDENT:** Meaning cannot be reliably assessed without specialized interpretation.
- **UNSUPPORTED:** No presently adequate evidence.
- **CONTRADICTED:** Available evidence materially undermines the proposition.

---

## Example Entry
| Field | Example |
|---|---|
| Proposition | Actor B performed a supporting role for Actor A’s property-related act. |
| Evidence | B’s message two days earlier, shared document template, payment from A, agency record showing B’s follow-up action. |
| Coordination indicator | Timing, shared document, payment, and complementary conduct converge. |
| Alternative explanation | B was independently retained for an ordinary administrative task. |
| Missing proof | Original message metadata and witness who can explain the payment purpose. |
| Status | **2 — Supported inference; not yet attribution proof.** |

---

The matrix should distinguish **coordination evidence** from **predicate-act evidence**: proving that several people interacted does not, by itself, prove that a racketeering act occurred or that the enterprise caused the plaintiff’s injury.

---

### Citations
[1] [PDF] CRIMINAL RICO: 18 U.S.C. §§1961-1968 A Manual For Federal ... https://www.justice.gov/d9/books/attachments/2018/12/07/_rico_manual_final_with_toa_05-12.pdf
[2] Rule 9. Pleading Special Matters - Law.Cornell.Edu https://www.law.cornell.edu/rules/frcp/rule_9
[3] A Fraudulent Scheme's Particularity Under Rule 9(b) of the Federal Rules of Civil Procedure https://digitalcommons.liberty.edu/cgi/viewcontent.cgi?article=1072&context=lu_law_review
[4] Article: Pleading Common Law Fraud Under Rule 9(b) https://www.quinnemanuel.com/the-firm/publications/article-may-2016-pleading-common-law-fraud-under-rule-9-b-conflicting-circuit-court-interpretations/
[5] [PDF] stringent particularity requirement of the rule 9(b) pleading standard as http://cardozolawreview.com/wp-content/uploads/2018/08/CHEN.36.1.pdf
[6] Quick Tips for Pleading “Fraud” with Particularity https://www.americanbar.org/groups/litigation/resources/newsletters/business-torts-unfair-competition/quick-tips-for-pleading-fraud-with-particularity/
[7] Specificity or Dismissal: The Improper Extension of Rule 9(b) to Negligent Misrepresentation as a Deprivation of Plaintiffs’ Procedural Due Process Rights https://scholarship.law.wm.edu/cgi/viewcontent.cgi?article=1699&context=wmborj
[8] "Proper Pleading or Premature Proof? Rule 9(b)'s Particularity ... https://digitalcommons.law.uga.edu/glr/vol49/iss3/6/
[9] [PDF] Rule 9(b) Proposal - United States Courts https://www.uscourts.gov/sites/default/files/20-cv-z_suggestion_from_dean_a._benjamin_spencer_-_rule_9_0.pdf
[10] Rule 9(b) Pleading Requirements Under the False Claims ... https://www.rivkinradler.com/publications/rule-9b-pleading-requirements-under-the-false-claims-act/
