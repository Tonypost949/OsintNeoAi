...
ZIP_PATH = r"G:\OsintNeoAi\takeout_chunks\takeout-20241121T010253Z-001.zip"
...
if os.path.exists(ZIP_PATH):
    print("[CHUNK 3] Extracting files...")
    with zipfile.ZipFile(ZIP_PATH, 'r') as zf:
        entries = zf.namelist()
        print(f"[CHUNK 3] Total entries: {len(entries)}")
        for item in entries:
...
tracker_data = {
    ...
    "current_batch": 3,
    "status": "CHUNK_3_COMPLETED"
}
with open(tracker_path, 'w') as tf:
    json.dump(tracker_data, tf, indent=2)

print("[CHUNK 3] Completed and updated chunk_progress_tracker.json!")
